...
## Notes: 
# Use dateInput() which brings up a calendar to select dates
...
ui <- navbarPage(
  ...
  # Make a graph that you can change using a slider
  #tabPanel("Simple X-Y Slider", 
  #         sliderInput(inputId = "x", label = "Slide X", min = 0, max = 100, 
  #                     value = 50),
  #         sliderInput(inputId = "y", label = "Slide Y", min = 0, max = 100, 
  #                     value = 50),
  ...
  # Show some plots
  tabPanel("Plots", 
           selectInput(inputId = "select", label = h3("Choose What to Plot:"),
                       choices = list("Tickets by Weekday" = "weekday", 
                                      "Tickets over Time" = "time"), 
                       selected = "weekday"),
           hr(),
  ...
  reactive({
    if(input$select[1] == "weekday")
    {
      output$Week = renderPlot({barplot(weekticket$freq, names.arg = weekticket$x, 
                                        ...
                                        main = "Tickets over Time")})
    }
    if(input$select[1] == "time")
    {
      output$Day = renderPlot({plot(ticketsDay, type = "b", 
                                    ylab = "Number of Tickets", 
                                    main = "Tickets by Weekday", las=2)})
    }
  })
